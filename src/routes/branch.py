...
@branch_bp.route('/', methods=['GET', 'POST'])
def get_branches():
    """عرض جميع الفروع"""
    branches = Branch.query.all()
    return render_template('branches/index.html', branches=branches)

@branch_bp.route('/api/list', methods=['GET', 'POST'])
...
def update_branch(branch_id):
    ...
    if data.get('location_latitude'):
        branch.location_latitude = float(data.get('location_latitude'))
    if data.get('location_longitude'):
        branch.location_longitude = float(data.get('location_longitude'))
